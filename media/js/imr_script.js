...
function funcFormSubmit(){
	//alert(issued.length);
	var count = 0;
	var receive = document.getElementById("rForm").value;
	var iDate = document.getElementById("datepicker").value;
	//alert(receive);
	...
	if(issued.length==0){
		...
		//alert("clickMe");
		var theForm, newInput1, newInput2, newInput3, newInput4, newInput5 ,check = true;
		...
		for(var i=0;i<issued.length;i++){
			...
			var id = issued[i];
			//alert(id);
			var Qty = parseFloat(issuedNumber[id]);
			//alert(Qty);
			if (isNaN(Qty)){
				//alert(Qty);
				check = false;
			...
			newInput1 = document.createElement('input');
				...
				newInput1.name = 'IMID'+count;
				newInput1.value = id;
				theForm.appendChild(newInput1);
			
			newInput2 = document.createElement('input');
				newInput2.type = 'hidden';
				newInput2.name = 'Qty'+count;
				newInput2.value = Qty.toFixed(2);
		...
		newInput4 = document.createElement('input');
				...
				newInput4.name = 'receive';
				newInput4.value = receive;
				theForm.appendChild(newInput4);
				
		newInput5 = document.createElement('input');
				newInput5.type = 'hidden';
				newInput5.name = 'impDate';
				newInput5.value = iDate;
...
// 	//alert(issued.length);
// 	var count = 0;
// 	var issueTo = document.getElementById("rFrom").value;
// 	//alert(issueTo);
// 	
// 	if(issued.length==0){
// 		
...
// 				newInput2.value = issueQty.toFixed(2);
// 				theForm.appendChild(newInput2);
// 		}//------end for loop------//
// 		
// 		newInput3 = document.createElement('input');
...
function funcValidQty(value,id,max,name){
	...
	if(value.trim()!=""){
		if (!isNaN(value)) 
		{
			...
			var oMax = parseFloat(max)-10;

			if(issue>maxValue){
				document.getElementById(id).value = oMax;
			}
