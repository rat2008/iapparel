<?php 
class tblbuyer_invoice_payment_category{
	private $conn;
	private $table_name  = "tblbuyer_invoice_payment_category";
	private $handle_misc = "";
	private $handle_tblcarton_inv_payment_head = "";
	...
	public function setTblcarton_inv_payment_head($handle_tblcarton_inv_payment_head){
		$this->handle_tblcarton_inv_payment_head = $handle_tblcarton_inv_payment_head;
	}
	...
	function duplicateDataFromCommercialToBuyerPayment(){
		...
		$sql = "SELECT * 
				...
				WHERE bic.invID=:invID AND bic.del=0";
		$stmt  = $this->conn->prepare($sql);
		$stmt->bindParam(":invID", $this->invID);
		$stmt->execute();
		while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
			...
			$arr_result = $this->checkCategoryExistNotDel(); // check category whether exist
			...
			if($isExist==0){// create new BICID
				...
				$this->custom_procedure = $custom_procedure;
				$BICID = $this->create();
	...
	function deleteAllRelatedRow(){
		...
		while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
			...
			$sqldel = "UPDATE tblbuyer_invoice_payment_detail SET del=1, delBy=:delBy, delDate=now()
							WHERE shipmentpriceID='$spID' AND invID='$invID' AND BICID='$BICID'";
			$deletesql = $this->conn->prepare($sqldel);
			$deletesql->bindParam(":delBy", $this->delBy);
			$deletesql->execute();
			
			$sql = "SELECT * FROM tblbuyer_invoice_payment_detail WHERE shipmentpriceID='$spID' AND invID='$invID' AND group_number>0 AND del=0";
			$stmt_chk = $this->conn->prepare($sql);
			$stmt_chk->execute();
			$count_sp = $stmt_chk->rowCount();
			
			if($count_sp==0){// remove Buyer PO other charge 
				$deletesql = $this->conn->prepare("UPDATE tblbuyer_invoice_payment_detail 
			...
			$this->handle_tblcarton_inv_payment_head->shipmentpriceID = $spID;
			$this->handle_tblcarton_inv_payment_head->invID = $invID;
			$this->handle_tblcarton_inv_payment_head->BICID = $BICID;
			$this->handle_tblcarton_inv_payment_head->removeByShipmentID();
		}
