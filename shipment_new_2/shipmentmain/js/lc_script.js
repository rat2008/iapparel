...
function funcSubmitDetail(count,LCIID){
	...
	console.log("LCIID: "+LCIID+" / buyer: "+buyer+" / str: "+str_selectdetail);
	$.ajax({
		url:"lc_ajax_custom.php",
		method:"post",
		data:{
...
function calPOAmt(count_info){
	...
	for(var i=1;i<countDet;i++){
		...
		if($("#po_amt_"+count_info+"-"+i).val() !==""){
			...
			amtship = parseFloat($("#draw_amt_"+count_info+"-"+i).val());
		}
		// alert(shipdate);
		arr_poamt.push(amt);
		arr_poamtship.push(amtship);
	...
	for(var i=1;i<=maxDate;i++){
		...
		if(str_lcdate!="" && expired_ele!=null){
			...
			for(var j=0;j<arr_poamt.length;j++){
				...
				var amtship  = parseFloat(arr_poamtship[j]);
				...
				$("#totalposhipamt"+count_info+"_"+i).html(sumship.toFixed(2)); //unassignlcprice
				$("#lcprice_txt"+count_info+"_"+i).val(sum.toFixed(2));
				$("#lcpriceship_txt"+count_info+"_"+i).val(sumship.toFixed(2));
...
function calDeviation(count_info,count_date){
	...
	var poamtship = parseFloat($("#totalposhipamt"+count_info+"_"+count_date).html());
	...
	if((limit!==0 && poamt!==0) && $("#limit_amt"+count_info+"_"+count_date).val()!==""){
		...
		var devship = (poamtship/limit)*100;
		devship += 0.00000000001;
		devship = devship.toFixed(2);

		$("#deviation"+count_info+"_"+count_date).html(dev+" %");
		$("#deviationship"+count_info+"_"+count_date).html(devship+" %");
		...
		if(devship>100){
			$("#deviationship"+count_info+"_"+count_date).css("color","red");
		}
		else if(devship<100){
			$("#deviationship"+count_info+"_"+count_date).css("color","orange");
		}
		else{
			$("#deviationship"+count_info+"_"+count_date).css("color","green");
		}
	}else{
...
function calAllPODeviation(count_info){ 
	//-- Calculate Total Limit, PO Amt, Deviation, PO number, unassigned PO by ckwai on 2021-01-25 --//
	var count_detail = parseInt($("#count_detail"+count_info).val());
	var maxDate      = parseInt($("#countDate"+count_info).val());
	var grand_limit_amt = 0;
	var grand_lcprice   = 0;
	var grand_totalpo   = 0;
	var grand_totalpoship = 0;
	var grand_allpo     = 0;
	var grand_allpoamt  = 0;
	var grand_allpoamtship = 0;
	...
	for(var i=1;i<count_detail;i++){
		...
		var draw_amt = document.getElementById("draw_amt_"+count_info+"-"+i);
		console.log(shipdate+" / "+i);
		if(shipdate!=null){
			grand_allpo++;
			
			var this_po_amt   = (po_amt.value==""? 0: po_amt.value);
			var this_draw_amt = (draw_amt.value==""? 0: draw_amt.value);
			grand_allpoamt += parseFloat(this_po_amt);
			grand_allpoamtship += parseFloat(this_draw_amt);
		}
	}
	...
	for(var i=1;i<=maxDate;i++){
		...
		var totalposhipamt = document.getElementById("totalposhipamt"+count_info+"_"+i);
		if(limit_amt!=null){
			...
			grand_totalpoship += parseFloat(totalposhipamt.innerHTML); 
		}
	}//--- End For ---//
	
	var grand_unassign    = grand_allpo - grand_totalpo;
	var grand_unassignamt = grand_allpoamt - grand_lcprice;
	var grand_unassignamtship = grand_allpoamtship - grand_totalpoship;
	
	grand_allpoamt  = Math.round((grand_allpoamt + Number.EPSILON) * 100) / 100;
	grand_limit_amt = Math.round((grand_limit_amt + Number.EPSILON) * 100) / 100;
	...
	document.getElementById("unassignlcpriceship"+count_info).innerHTML = grand_unassignamtship.toFixed(2);
	document.getElementById("lcpriceship"+count_info).innerHTML = grand_allpoamtship.toFixed(2);
	
	var dev = (grand_allpoamt / (grand_limit_amt)) * 100;
		dev += 0.00000000001;
		dev = dev.toFixed(2);
	
	var devship = (grand_allpoamtship / grand_limit_amt) * 100;
		devship += 0.00000000001;
		devship = devship.toFixed(2);
	
	document.getElementById("deviation"+count_info).innerHTML = dev+" %";
	document.getElementById("deviationship"+count_info).innerHTML = devship+" %";
	...
	if(devship>100){
		$("#deviationship"+count_info).css("color","red");
	}
	else if(devship<100){
		$("#deviationship"+count_info).css("color","orange");
	}
	else{
		$("#deviationship"+count_info).css("color","green");
	}
