...
class TCPDF {
	...
	public function Error($msg) {
		...
		if (defined('K_TCPDF_THROW_EXCEPTION_ERROR') AND !K_TCPDF_THROW_EXCEPTION_ERROR) {
			die('<strong>TCPDF ERROR2: </strong>'.$msg);
		} else {
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		switch($dest) {
			case 'I': {
				...
				if (ob_get_contents()) {
					$this->Error('1.....Some data has already been output, can\'t send PDF file');
				}
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		if ($this->inxobj) {
			...
			if (isset($this->footerlen[$this->page])) {
				...
				$this->footerpos[$this->page] = (isset($this->pagelen[$this->page])? $this->pagelen[$this->page]: 1);
			}
		...
		while ($key < $maxel) {
			...
			if ($dom[$key]['tag'] OR ($key == 0)) {
				...
				$curpos = (isset($this->pagelen[$startlinepage])? $this->pagelen[$startlinepage]: 1);
